# Rule — Dossier Schema (data contract)

Always-active. The dossier is the sole inter-stage channel (constitution Principle 8). Each stage reads prior
sections and writes its own. NOTE: the managed agent does not enforce structured output — the agent MAINTAINS
this shape as its state contract, and the code-level validations (bundled skill scripts) verify the
safety/number-critical fields. Conformance is agent-maintained + code-verified, not a provider guarantee.

Annotation tags: [safety] re-derived on restore, never trusted from file; [SENSITIVE] see sensitive-data rule;
[computed] deterministic tool output; [narrative] model prose; [user] family-provided; [SECURITY] untrusted.
